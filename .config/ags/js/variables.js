...
export const temp = Variable(0, {
  poll: [options.systemFetchInterval, 'sensors', out => {
    const temperatureInfo = out.split('\n')
      .find(line => line.includes('CPUTIN'))
      .match(/CPUTIN:\s+(\S+)°C/);
    if (temperatureInfo) {
      const temperatureValue = parseFloat(temperatureInfo[1].replace(',', '.'));
      const formattedTemperature = temperatureValue.toFixed(1);
